...
Below, I (1) create an object only with forest pixels to calculate forest cover and the number of forest patches (habitat), and (2) create a new raster with all land-use types except forest (non-habitat)

The reference code for land-use type and pixel values is [here](https://brasil.mapbiomas.org/wp-content/uploads/sites/4/2023/08/Legenda-Colecao-8-LEGEND-CODE-1.pdf).
...
After the random selection, I manually adjusted some points in QGIS to better represent the landscapes. For example, if one or two out of ten points in a small landscape fell within a large fragment, I moved those points to smaller fragments. Later, in the field, I had to adjust some points because they fell in clearings. However, I followed the same rules (100 m from the landscape edge and 50 m from the fragment edge).
...
ants_coord <- ants[-c(201:240),] #removing the points in the continuous forest in landscapes with ~100% FC
